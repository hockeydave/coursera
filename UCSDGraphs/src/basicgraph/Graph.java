...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		List<Integer> sequenceDegrees = new ArrayList<Integer>();
		for(int i = 0; i < numVertices; i++) {
			sequenceDegrees.add(getNeighbors(i).size()+ getInNeighbors(i).size());
		}
		Collections.sort(sequenceDegrees, Collections.reverseOrder());
		return sequenceDegrees;
	}
	 ...
	 * with a given index.
	 * @param v index of vertex being checked
	...
	public static void main (String[] args) {
		...
		List<Integer> ds = airportGraph.degreeSequence();
		int over100 = 0;
		for(int i = 0; i < ds.size(); i++) {
			if(ds.get(i) >= 100)
				over100++;
		}
		System.out.println("Observe most degrees are small (1-30), eight are over 100. " + over100);
		System.out.println("****");
		...
		// GraphAdjList.
		System.out.println("data/testdata/simpletest.map...");
		for(int i = 0; i < graphFromFile.getNumVertices(); i++) {
			System.out.println(graphFromFile.getDistance2(i));
		}
		System.out.println("airportGraph...");
		for(int i = 0; i < graphFromFile.getNumVertices(); i++) {
			System.out.println(airportGraph.getDistance2(i));
		}
